...
def parse(fn_or_dn: str, relative_to: str) -> Dict[str, Unit]:
    if os.path.isfile(fn_or_dn):
        return parse_file(fn_or_dn, relative_to)
    elif os.path.isdir(fn_or_dn):
        return parse_dir(fn_or_dn, relative_to)
    else:
        # f'{fn_or_dn} is not a file or a directory.'
        pass


def parse_dir(dn: str, relative_to: str) -> Dict[str, Unit]:
    rv = {}
    for root, _, files in os.walk(dn):
        for fn in files:
            fn = os.path.join(root, fn)
            rv.update(parse_file(fn, relative_to))
    return rv


def parse_file(fn: str, relative_to: str) -> Dict[str, Unit]:
